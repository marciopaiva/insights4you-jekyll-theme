...
build: check-deps ## Build the Jekyll theme gem
	...
	@cd "$(THEME_DIR)" && $(BUNDLE) exec gem build "$(GEM_NAME).gemspec" 2>&1 | tee -a $(LOG_FILE)
	@$(BUNDLE) exec gem install "./$(GEM_NAME)-*.gem" 2>&1 | tee -a $(LOG_FILE)
	@rm -f "./$(GEM_NAME)-*.gem"
...
remove-old-gems: ## Remove old versions of the theme gem
	...
	@$(BUNDLE) exec gem list "$(GEM_NAME)" --local | grep -oP "$(GEM_NAME) \(\K[^\)]+" | tr ',' '\n' | \
	sort -V | head -n -1 | xargs -I {} $(BUNDLE) exec gem uninstall "$(GEM_NAME)" -v {} --executables --force || true
...
install: check-deps ## Install dependencies in example site
	...
	@cd "$(EXAMPLE_DIR)" && $(BUNDLE) install 2>&1 | tee -a $(LOG_FILE)
...
prod: build remove-old-gems kill-services install ## Build and serve the site in production mode
	@printf "$(YELLOW)-----------------------------------------------------------------------------------$(NC)\n"
	...
	@printf "$(YELLOW)-----------------------------------------------------------------------------------$(NC)\n"
	@cd "$(EXAMPLE_DIR)" && JEKYLL_ENV=production $(JEKYLL) serve \
...
dev: build remove-old-gems kill-services install ## Serve with live reload (development mode)
	...
	@printf "$(BLUE)Server URL: http://localhost:$(JEKYLL_PORT)$(NC)\n"
	@printf "$(YELLOW)-----------------------------------------------------------------------------------$(NC)\n"
	@cd "$(EXAMPLE_DIR)" && JEKYLL_ENV=development $(JEKYLL) serve \
